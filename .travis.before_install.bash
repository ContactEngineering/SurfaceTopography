...
if [ "$WITH_MPI" == "yes" ]; then
  sudo apt-get install openmpi-bin libopenmpi-dev libfftw3-mpi-dev libhdf5-mpi-dev libnetcdf-pnetcdf-dev
fi
...
if [ "$WITH_MPI" == "yes" ]; then
  python -m pip install --force-reinstall --no-binary mpi4py mpi4py==${MPI4PY_VERSION}
