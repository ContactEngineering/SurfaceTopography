...
import pytest
from SurfaceTopography.IO.binary import BinaryStructure, Validate, Convert
from SurfaceTopography.IO.Reader import CompoundLayout, If, Skip, SizedChunk, For
...
def test_binary_structure_to_dict():
    ...
    assert d['structure'][1]['convert'] == {'type': 'Convert', 'fun': 'callable_convert'}
...
def test_binary_structure_with_lambdas_to_dict():
    ...
    assert d['structure'][0]['validate'] == {'type': 'lambda', 'source': 'callable_validator'}
    assert d['structure'][1]['convert'] == {'type': 'lambda', 'source': 'callable_converter'}
